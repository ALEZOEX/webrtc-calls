...
io.on('connection', (socket) => {
  ...
  socket.on('BE-join-room', ({ roomId, userName, password }) => {
    ...
    if (isPrivateRoom) {
      // Если комната уже существует и является приватной, проверяем пароль
      if (!password || roomPasswords.get(roomId) !== password) {
        socket.emit('FE-error', { error: 'Неверный пароль для приватной комнаты' });
        return;
      }
    } else if (password) {
      // Если комната еще не существует и пароль передан, сохраняем его (для новой приватной комнаты)
      // roomPasswords.set(roomId, password);
      console.log(`🔒 Пароль для новой приватной комнаты будет установлен: ${roomId}`);
    }
    ...
    } else if (!roomForCheck && !password) {
      // Если комната публичная - не сохраняем пароль
      console.log(`🌐 Создана публичная комната: ${roomId}`);
    }
